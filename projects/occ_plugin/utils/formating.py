...
def cm_to_ious(cm):
    ...
    for i in range(cls_num):
        ...
        union = p + g - tp
        iou = tp / union
        print(f'cls: {i}, tp: {tp}, p: {p}, g: {g}, union: {union}, iou: {iou}')
...
def format_iou_results(mean_ious, return_dic=False):
    ...
    for cls_name, cls_iou in zip(class_names, class_ious):
        dic[cls_name] = np.round(cls_iou, 4)
        x.add_row([cls_name, np.round(cls_iou, 4)])
    
    mean_ious = sum(mean_ious[1:]) / len(mean_ious[1:])
    dic['mean'] = np.round(mean_ious, 4)
    x.add_row(['mean', np.round(mean_ious, 4)])
