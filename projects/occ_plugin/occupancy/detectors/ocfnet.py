...
class OCFNet(BEVDepth):
    ...
    def image_encoder(self, img):
        imgs = img
        B, N, C, imH, imW = [int(ss) for ss in imgs.shape]
        ...
        if self.with_img_neck:
            ...
            x = backbone_feats
        _, output_dim, ouput_H, output_W = [int(ss) for ss in x.shape]
    ...
    def occ_encoder(self, x):
        b, t, _, _, _, _ = [int(ss) for ss in x.shape]
        x = x.reshape(b, -1, *[int(ss) for ss in x.shape[3:]])
    ...
    def forward_train(self,
        ...
        # segmentation: inflated, only for movable objects?
        ...
        for voxel_feats_stage in voxel_feats:
            bs, sfeatures = [int(ss) for ss in voxel_feats_stage.shape[:2]]
            voxel_feats_stage_ = voxel_feats_stage.view(bs*self.n_future_frames_plus, sfeatures//self.n_future_frames_plus, *[int(ss) for ss in voxel_feats_stage.shape[2:]])
        ...
        for flow_feats_stage in flow_feats:
            bs, sfeatures = [int(ss) for ss in flow_feats_stage.shape[:2]]
            flow_feats_stage_ = flow_feats_stage.view(bs*self.n_future_frames_plus, sfeatures//self.n_future_frames_plus, *[int(ss) for ss in flow_feats_stage.shape[2:]])
