...
# https://github.com/JLChen-C/OccProphet
...
class OccProphet(BEVDepth):

    def __init__(self, 
                 loss_cfg=None,
                 only_generate_dataset=False,
                 disable_loss_depth=False,
                 test_time_indices=None,
                 test_present=False,
                 test_present_and_future=False,
                 empty_idx=0,
                 max_label=2,
        ...
        '''
        super().__init__(pts_bbox_head=pts_bbox_head, **kwargs)
        ...
        if shared_predictor is not None:
            self.shared_predictor = builder.build_neck(shared_predictor)
            self.multiscale_output = self.shared_predictor.multiscale_output
        else:
            self.shared_predictor = None
            self.occ_predictor = builder.build_neck(occ_predictor) if occ_predictor else nn.Identity()
            self.flow_predictor = builder.build_neck(flow_predictor) if flow_predictor else nn.Identity()

            if hasattr(self.occ_predictor, 'multiscale_output') and hasattr(self.flow_predictor, 'multiscale_output'):
                assert self.occ_predictor.multiscale_output == self.flow_predictor.multiscale_output
            self.multiscale_output = getattr(self.occ_predictor, 'multiscale_output', False)
        ...
        task_list = ['inflated GMO', 'fine-grained GMO', 'inflated GMO, fine-grained GSO', 'fine-grained GMO, fine-grained GSO']
        num_classes_list = [2, 2, 3, 3]
        target_num_classes = num_classes_list[task_list.index(task)]
        assert task in task_list
        assert max_label == target_num_classes, f'max_label={max_label} is not equal to target num_classes={target_num_classes}'
        self.task = task

        if (test_time_indices is not None) and (not isinstance(test_time_indices, (tuple, list))):
            test_time_indices = [test_time_indices]
        self.test_time_indices = test_time_indices
        self.test_present = test_present
        self.test_present_and_future = test_present_and_future
        self.save_pred = save_pred
        self.save_path = save_path

        logger = get_root_logger(name='mmdet')
        logger.info(f'test_time_indices: {test_time_indices}')
        logger.info(f'test_present: {test_present}, test_present_and_future: {test_present_and_future}, self.eval_start_moment: {self.eval_start_moment}')
        n_params = sum(p.numel() for p in self.parameters())
        logger.info(f'Total Params: {n_params/1e6}M (1e6), {n_params/(1024*1024)}M (1024*1024)')
        time.sleep(3)

    @property
    def with_flow(self):
        """bool: Whether the detector has a 3D box flow head."""
        return hasattr(self,
                       'flow_head') and self.flow_head is not None

    def image_encoder(self, img):
        imgs = img
        B, N, C, imH, imW = [int(s) for s in imgs.shape]
        ...
        if self.with_img_neck:
            ...
            x = backbone_feats
        _, output_dim, ouput_H, output_W = [int(s) for s in x.shape]
    ...
    def forward_encoder(self, voxel_feats, encoder):
        encoder = getattr(self, encoder)
        feats_list = encoder(voxel_feats)

        future_vox_feats_inter, future_vox_feats = feats_list

        if self.multiscale_output:
            if future_vox_feats_inter is not None:
                for i in range(len(future_vox_feats_inter)):
                    if future_vox_feats_inter[i].ndim == 6:
                        future_vox_feats_inter[i] = future_vox_feats_inter[i].flatten(1, 2)
        else:
            future_vox_feats = future_vox_feats.flatten(1, 2)
            if (future_vox_feats_inter is not None) and (future_vox_feats_inter.ndim == 6):
                future_vox_feats_inter = future_vox_feats_inter.flatten(1, 2)
    ...
    def pack_dbatch_and_dtime(self, x):
        b = int(x.shape[0])
        s = int(x.shape[1])
        x = x.view(b*s, *[int(ss) for ss in x.shape[2:]])
        return x
    
    def unpack_dbatch_and_dtime(self, x, b, s):
        assert (b*s) == int(x.shape[0])
        x = x.view(b, s, *[int(ss) for ss in x.shape[1:]])
        return x
    ...
    def extract_img_feat(self, img_inputs_seq, img_metas):
        ...
        self.batch_size = int(imgs_seq.shape[0])
        self.sequence_length = int(imgs_seq.shape[1])
        ...
        self.n_cam = int(imgs_seq.shape[1])
    ...
    def warp_features(self, x, flow, tseq):
        ...
        b, dc, dx, dy, dz = [int(ss) for ss in x.shape]
        x_dtype = x.dtype
        flow_dtype = flow.dtype
        if x_dtype is not torch.float:
            x = x.float()
        if flow_dtype is not torch.float:
            flow = flow.float()
        ...
        grid = grid.view(dx*dy*dz, int(grid.shape[-1])).unsqueeze(-1)   #[N,4,1]
        ...
        transformed_grid = transformed_grid[:, :, :, 0] # [bs, N, 4]
        ...
        # newly added
        grid = grid.unsqueeze(0).expand(b, -1, -1).reshape(-1, 4)

        batch_ix = torch.cat([torch.full([int(transformed_grid.shape[0]) // b, 1], ix, device=x.device, dtype=torch.long) for ix in range(b)])
        ...
        warped_x =  x.new_zeros([int(ss) for ss in x.shape])
        for i in range(int(transformed_grid.shape[1])):  
        ...
        if warped_x.dtype is not x_dtype:
            warped_x = warped_x.to(x_dtype)
    ...
    def extract_feat(self, img_inputs_seq, img_metas, future_egomotion):
        ...
        batch_size, sequence_length, nbr_pose_channels = [int(ss) for ss in future_egomotion_vec.shape]
        dx, dy, dz = [int(ss) for ss in voxel_feats.shape[-3:]]
        ...
        if self.shared_predictor is not None:
            vox_feats_list = self.forward_encoder(voxel_feats, 'shared_predictor')
            if self.record_time:
                torch.cuda.synchronize()
                t2 = time.time()
                self.time_stats['shared_encoder'].append(t2 - t1)

            for i in range(len(vox_feats_list)):
                if type(vox_feats_list[i]) is not list:
                    vox_feats_list[i] = [vox_feats_list[i]]

            vox_feats_inter, vox_feats = vox_feats_list
            vox_feats = flow_feats = vox_feats
            vox_feats_inter = flow_feats_inter = vox_feats_inter
        else:
            vox_feats_list = self.forward_encoder(voxel_feats, 'occ_predictor')
            if self.record_time:
                torch.cuda.synchronize()
                t2 = time.time()
            ...
            flow_feats_list = self.forward_encoder(voxel_feats, 'flow_predictor')
            if self.record_time:
                torch.cuda.synchronize()
                t3 = time.time()
                self.time_stats['flow_encoder'].append(t3 - t2)
            
            for i in range(len(flow_feats_list)):
                if type(flow_feats_list[i]) is not list:
                    flow_feats_list[i] = [flow_feats_list[i]]

            flow_feats_inter, flow_feats = flow_feats_list

        depth = depth.view(-1, self.n_cam, *[int(ss) for ss in depth.shape[-3:]])
        return vox_feats_inter, vox_feats, flow_feats_inter, flow_feats, img_feats, depth
    ...
    def forward_occ_train(
        ...
        head_name = head.replace('pts_bbox', 'occ')
        loss_name = 'loss_' + head_name.replace('_head', '')
        ...
        if self.record_time:
            torch.cuda.synchronize()
            t1 = time.time()
            self.time_stats[head_name].append(t1 - t0)
        
        losses = head_module.loss(
        ...
        if self.record_time:
            torch.cuda.synchronize()
            t2 = time.time()
            self.time_stats[loss_name].append(t2 - t1)
    ...
    def forward_train(self,
        ...
        # ['inflated GMO', 'fine-grained GMO', 'inflated GMO, fine-grained GSO', 'fine-grained GMO, fine-grained GSO']
        # gt_occ: fine-grained movable [and static objects] when enabling fine-grained occ
        if self.task == 'inflated GMO':
            gt_occ = segmentation
        elif self.task == 'fine-grained GMO':
            gt_occ = gt_occ
        elif self.task == 'inflated GMO, fine-grained GSO':
            gt_occ = gt_occ
            gt_occ[segmentation == 1] = 1
        elif self.task == 'fine-grained GMO, fine-grained GSO':
            gt_occ = gt_occ

        feats_list = self.extract_feat(
            img_inputs_seq=img_inputs_seq, img_metas=img_metas, future_egomotion=future_egomotion)
        vox_feats_inter, vox_feats, flow_feats_inter, flow_feats, img_feats, depth = feats_list
        ...
        if not self.disable_loss_depth and depth is not None:
            ...
            depth_gt = depth_gt.view(int(depth_gt.shape[0])*int(depth_gt.shape[1]),*[int(ss) for ss in depth_gt.shape[2:]])
            depth = depth.view(-1, *[int(ss) for ss in depth.shape[2:]])
        ...
        (vox_feats, flow_feats) = map(
            lambda x_list: [x.reshape(
                int(x.shape[0])*self.n_future_frames_plus, int(x.shape[1])//self.n_future_frames_plus, *[int(ss) for ss in x.shape[2:]])
                for x in x_list],
            [vox_feats, flow_feats]
        )
        
        losses_occ = self.forward_occ_train('pts_bbox_head', vox_feats, gt_occ,
                        points_occ, img_metas, img_feats=img_feats, transform=transform)
        losses.update(losses_occ)

        losses_flow = self.forward_occ_train('flow_head', flow_feats, flow,
                        points_occ, img_metas, img_feats=img_feats, transform=transform)
    ...
    def forward_test(self,
        ...
        assert img_inputs_seq[0].shape[0] == 1
    ...
    def simple_test(self, img_metas, img_inputs_seq=None, rescale=False, points_occ=None, 
        ...
        if not return_raw_pred:
            ...
            gt_occ = gt_occ.view(int(gt_occ.shape[0])*int(gt_occ.shape[1]), *[int(ss) for ss in gt_occ.shape[2:]])
            instance = instance[:, -self.n_future_frames_plus:, ...].contiguous()
            instance = instance.view(int(instance.shape[0])*int(instance.shape[1]), *[int(ss) for ss in instance.shape[2:]])

        output = self.pts_bbox_head(
            voxel_feats=vox_feats,
            points=points_occ,
            img_metas=img_metas,
            img_feats=img_feats,
            transform=transform,
        ...
        if not return_raw_pred:
            ...
            gt_flow = gt_flow.view(int(gt_flow.shape[0])*int(gt_flow.shape[1]), *[int(ss) for ss in gt_flow.shape[2:]])

        pred_flow = output_flow['output_voxels'][0]

        if return_raw_pred:
            return dict(output_voxels=pred_c, output_flows=pred_flow)

        vpq = self.evaluate_instance_prediction(pred_c, pred_flow, gt_occ, instance)
        # vpq = 0.1

        if self.test_time_indices is not None:
            pred_c = pred_c[self.test_time_indices, ...]
            gt_occ = gt_occ[self.test_time_indices, ...]
        elif self.test_present:
        ...
        elif self.test_present_and_future:
            pred_c = pred_c[self.eval_start_moment:, ...]
            gt_occ = gt_occ[self.eval_start_moment:, ...]
        else:
    ...
    def evaluate_occupancy_forecasting(self, pred, gt, img_metas=None, save_pred=False, save_path=None):
        ...
        B, H, W, D = [int(ss) for ss in gt.shape]
        if [int(ss) for ss in pred.shape[-3:]] != [int(ss) for ss in gt.shape[-3:]]:
        ...
        if save_pred:
            ...
            pred_path = os.path.join(save_path, img_metas[0]["scene_token"])
            if not os.path.isfile(pred_path):
                np.savez(pred_path, pred_for_save_list)
    ...
    def group_pixels(self, centers: torch.Tensor, offset_predictions: torch.Tensor) -> torch.Tensor:
        dx, dy, dz = [int(ss) for ss in offset_predictions.shape[-3:]]
        x_grid = (
    ...
    def get_instance_segmentation_and_centers(self,
        ...
        if not len(centers):
            return torch.zeros([int(ss) for ss in center_predictions.shape], dtype=torch.int64, device=center_predictions.device)
    ...
    def flow_warp(self, occupancy, flow, mode='nearest', padding_mode='zeros'):
        ...
        _, num_waypoints, _, grid_dx_cells, grid_dy_cells, grid_dz_cells = [int(ss) for ss in occupancy.size()]
    ...
    def make_instance_id_temporally_consecutive(self, pred_inst, preds, backward_flow, ignore_index=255.0):
        ...
        seq_len, _, dx, dy, dz = [int(ss) for ss in preds.shape]
    ...
    def evaluate_instance_prediction(self, pred_seg, pred_flow, gt_seg, gt_instance):
        ...
        B, H, W, D = [int(ss) for ss in gt_seg.shape]
        ...
        unique_id_mapping = {}
        for t in range(int(pred_segmentation.shape[0])):
    ...
    def forward_dummy(self,
        ...
        # rots_seq, trans_seq, intrins_seq, post_rots_seq, post_trans_seq, gt_depths_seq, sensor2sensors_seq
        # (B, T, n_cam, M, N)
        B, T, N = [int(ss) for ss in img_inputs.shape[:3]]
        dummy_params_shape = (
            (B, T, N, 3, 3), (B, T, N, 3), (B, T, N, 3, 3), (B, T, N, 3, 2), (B, T, N, 3), (B, T, N, 1, 1), (B, T, N, 3, 4)
        )
        dummy_params = [img_inputs.new_zeros(s) for s in dummy_params_shape]
        dummy_params[2] = torch.eye(3)[None, None, None].repeat(B, T, N, 1, 1).to(img_inputs)
        dummy_params[3] = torch.eye(3)[None, None, None].repeat(B, T, N, 1, 1).to(img_inputs)
        future_egomotion = img_inputs.new_zeros(B, T, 4, 4)
        img_inputs = [img_inputs] + dummy_params
        feats_list = self.extract_feat(
            img_inputs_seq=img_inputs, img_metas=img_metas, future_egomotion=future_egomotion)
        vox_feats_inter, voxel_feats, flow_feats_inter, flow_feats, img_feats, depth = feats_list

        voxel_feats = voxel_feats[0]
        voxel_feats = voxel_feats.view(
            int(voxel_feats.shape[0])*self.n_future_frames_plus, int(voxel_feats.shape[1])//self.n_future_frames_plus, *[int(ss) for ss in voxel_feats.shape[2:]])
        voxel_feats = [voxel_feats]
