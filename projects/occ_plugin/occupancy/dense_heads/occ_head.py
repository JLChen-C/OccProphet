...
class OccHead(nn.Module):
    ...
    def loss_voxel(self, output_voxels, target_voxels, tag):
        ...
        tB, tC, tH, tW, tD = [int(ss) for ss in target_voxels.shape]
        if [H, W, D] != [tH, tW, tD]:
            output_voxels = F.interpolate(output_voxels, [tH, tW, tD], mode='trilinear', align_corners=False)

        target_voxels = target_voxels.reshape(tB*tC, tH, tW, tD)

        with torch.no_grad():
            flag = torch.isnan(output_voxels).any().item() or torch.isnan(target_voxels).any().item()
        
        if flag:
            return {
                'loss_voxel_ce_{}'.format(tag): output_voxels.sum() * 0.
            }
    ...
    def loss_point(self, fine_coord, fine_output, target_voxels, tag):
        ...
        if flag:
            ...
            return {
                ...
                'loss_voxel_sem_scal_{}'.format(tag).format(tag): loss_holder,
