...
def cumsum_trick(x, geom_feats, ranks):
    x = x.cumsum(0)
    kept = torch.ones(int(x.shape[0]), device=x.device, dtype=torch.bool)
...
class ViewTransformerLiftSplatShoot(BaseModule):
    def __init__(self, grid_config=None, data_config=None,
                ...
                numC_input=512, numC_mid=None, numC_Trans=64, downsample=16,
        ...
        self.D, _, _, _ = [int(ss) for ss in self.frustum.shape]
        self.numC_input = numC_input
        self.numC_mid = numC_mid or numC_input
    ...
    def create_frustum(self):
        ...
        D, _, _ = [int(ss) for ss in ds.shape]
    ...
    def get_geometry(self, rots_seq, trans_seq, intrins_seq, post_rots_seq, post_trans_seq, bda):
        ...
        B, N, _ = [int(ss) for ss in trans_seq.shape]
        ...
        dtype = post_rots_seq.dtype
        if dtype is not torch.float:
            post_rots_seq_float = post_rots_seq.float()
            post_rots_seq_inv = torch.inverse(post_rots_seq_float)
            post_rots_seq_inv = post_rots_seq_inv.to(dtype)
            points = post_rots_seq_inv.view(B, N, 1, 1, 1, 3, 3).matmul(points.unsqueeze(-1))
        else:
            points = torch.inverse(post_rots_seq).view(B, N, 1, 1, 1, 3, 3).matmul(points.unsqueeze(-1))
        ...
        dtype = intrins_seq.dtype
        if dtype is not torch.float:
            intrins_seq_float = intrins_seq.float()
            intrins_seq_inv = torch.inverse(intrins_seq_float)
            intrins_seq_inv = intrins_seq_inv.to(dtype)
            combine = rots_seq.matmul(intrins_seq_inv)
        else:
            combine = rots_seq.matmul(torch.inverse(intrins_seq))
    ...
    def voxel_pooling(self, geom_feats, x):
        B, N, D, H, W, C = [int(ss) for ss in x.shape]
        Nprime = B * N * D * H * W
    ...
    def voxel_pooling_accelerated(self, rots, trans, intrins, post_rots, post_trans, bda, x):
        ...
        if self.geom_feats is None:
            ...
            kept1 = (geom_feats[:, 0] >= 0) & (geom_feats[:, 0] < self.nx[0]) \
                    & (geom_feats[:, 1] >= 0) & (geom_feats[:, 1] < self.nx[1]) \
                    & (geom_feats[:, 2] >= 0) & (geom_feats[:, 2] < self.nx[2])
            idx = torch.range(0, int(x.shape[0]) - 1, dtype=torch.long)
            x = x[kept1]
            idx = idx[kept1]
            ...
            repeat_id = torch.ones(int(geom_feats.shape[0]), device=geom_feats.device, dtype=geom_feats.dtype)
            curr = 0
            repeat_id[0] = 0
            curr_rank = ranks[0]

            for i in range(1, int(ranks.shape[0])):
    ...
    def forward(self, input):
        ...
        B, N, C, H, W = [int(ss) for ss in x.shape]
        x = x.view(B * N, C, H, W)
        x = self.depth_net(x)
...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        x5 = F.interpolate(x5,
                           size=[int(ss) for ss in x4.size()[2:]],
                           mode='bilinear',
                           align_corners=True)
...
class DepthNet(nn.Module):
    def __init__(self, in_channels, mid_channels, context_channels,
                 depth_channels, cam_channels=27, norm_cfg=None, with_cp=False):
    ...
    def forward(self, x, mlp_input):
        ...
        def _forward(x, mlp_input):
            mlp_input = self.bn(mlp_input.reshape(-1, int(mlp_input.shape[-1])))
        ...
        if self.with_cp:
            return checkpoint(_forward, x, mlp_input)
        else:
            return _forward(x, mlp_input)
...
class ViewTransformerLSSBEVDepth(ViewTransformerLiftSplatShoot):
    def __init__(self, loss_depth_weight, cam_channels=27, loss_depth_reg_weight=0.0, use_voxel_net=False, 
                 norm_cfg=dict(type='BN2d', eps=1e-3, momentum=0.01), with_cp=False, **kwargs):
        ...
        self.depth_net = DepthNet(self.numC_input, self.numC_mid,
                                  self.numC_Trans, self.D, cam_channels=self.cam_channels,
                                  norm_cfg=norm_cfg, with_cp=with_cp)
        self.depth_aggregation_net = DepthAggregation(self.numC_Trans,
                                                      self.numC_mid,
    ...
    def _forward_voxel_net(self, img_feat_with_depth):
        ...
        n, h, c, w, d = [int(ss) for ss in img_feat_with_depth.shape]
    ...
    def get_downsampled_gt_depth(self, gt_depths):
        ...
        Output:
            gt_depths: [B*N*h*w, d]
        """
        B, N, H, W = [int(ss) for ss in gt_depths.shape]
...
class DepthNetStereo(nn.Module):
    ...
    # @autocast(False)
    def forward(self, x, mlp_input):
        B, _, H, W = [int(ss) for ss in x.shape]
...
class ViewTransformerLSSBEVStereo(ViewTransformerLSSBEVDepth):
    ...
    def create_depth_sample_frustum(self, depth_sample, downsample_factor=16):
        ...
        batch_size, num_depth, _, _ = [int(ss) for ss in depth_sample.shape]
    ...
    def homo_warping(
        ...
        """
        batch_size_with_num_cams, channels = int(stereo_feat.shape[
            0]), int(stereo_feat.shape[1])
        height, width = int(stereo_feat.shape[2]), int(stereo_feat.shape[3])
        with torch.no_grad():
            points = frustum
            points = points.reshape(int(points.shape[0]), -1, int(points.shape[-1]))
            points[..., 2] = 1
            # Undo ida for key frame.
            points = key_ida_mats.reshape(batch_size_with_num_cams,
                                          *[int(ss) for ss in key_ida_mats.shape[2:]]).inverse(
            ...
            num_depth = int(frustum.shape[1])
            points = (key_intrin_mats.reshape(
                batch_size_with_num_cams,
                *[int(ss) for ss in key_intrin_mats.shape[2:]]).inverse().unsqueeze(1) @ points)
            points = (sensor2sensor_mats.reshape(
                batch_size_with_num_cams,
                *[int(ss) for ss in sensor2sensor_mats.shape[2:]]).unsqueeze(1) @ points)
            # points in sweep sensor coord.
            points = (sweep_intrin_mats.reshape(
                batch_size_with_num_cams,
                *[int(ss) for ss in sweep_intrin_mats.shape[2:]]).unsqueeze(1) @ points)
            ...
            points = (sweep_ida_mats.reshape(
                batch_size_with_num_cams,
                *[int(ss) for ss in sweep_ida_mats.shape[2:]]).unsqueeze(1) @ points).squeeze(-1)
    ...
    def _generate_cost_volume(
        ...
        """
        batch_size, num_channels, height, width = [int(ss) for ss in stereo_feats_all_sweeps[
            0].shape]
    ...
    def _forward_stereo(
        ...
        batch_size_with_cams, _, feat_height, feat_width = \
            [int(ss) for ss in stereo_feats_all_sweeps[0].shape]
    ...
    def forward(self, input):
        ...
        B, N, C, H, W = [int(ss) for ss in img_feat.shape]
