...
class ViewTransformerLiftSplatShootVoxel(ViewTransformerLSSBEVDepth):
    def __init__(self, loss_depth_weight, loss_depth_type='bce', with_cp=False, **kwargs):
        super(ViewTransformerLiftSplatShootVoxel, self).__init__(loss_depth_weight=loss_depth_weight, with_cp=with_cp, **kwargs)
    ...
    def voxel_pooling(self, geom_feats, x):
        ...
        if dtype is not torch.float:
            x_float = x.float()
            final = occ_pool(x_float, geom_feats, B, self.nx[2], self.nx[0], self.nx[1])
