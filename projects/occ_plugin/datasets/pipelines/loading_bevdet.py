...
class LoadMultiViewImageFromFiles_BEVDet(object):
    ...
    def __init__(self, data_config, is_train=False, using_ego=True, colorjitter=False,
                 ...
                 mmlabnorm=False, load_depth=False, depth_gt_path=None, data_root=None, test_mode=False, use_lyft=False, save_case=False,):
    ...
    def get_seq_inputs(self, results, flip=None, scale=None):
        ...
        if self.test_mode and self.save_case:
            output_folder = f"/home/jlchen/code/occupancy/Cam4DOcc/Cam4DOcc/viz/images{results['sequence_length']}f/"
            if not os.path.exists(output_folder):
                os.makedirs(output_folder, exist_ok=True)

            case_folder = os.path.join(output_folder, results['scene_token'])
        ...
        for counter in range(sequence_length):
            ...
            if self.test_mode and self.save_case:
                frame_folder = os.path.join(case_folder, str(counter))
            ...
            for cam_idx, cam_name in enumerate(cam_names):
                ...
                img = Image.open(filename)
                if self.test_mode and self.save_case:
                    raw_image_path = os.path.join(frame_folder, f"{cam_name}." + filename.split('.')[-1])
                ...
                if True:
                # TODO add depth gt for lyft or using open source depth enhancement
                # if self.use_lyft:
                    gt_depths.append(torch.zeros(1))
                else:
                    cam_filename = os.path.basename(cam_data['data_path']).split('.')[0]
                    depth_file = os.path.join(self.depth_gt_path, 'samples', cam_name, cam_filename + '.npy')
                    depth_gt = np.load(depth_file)
                    depth_gt = Image.fromarray(depth_gt)
                    depth_gt = depth_gt.resize(resize_dims, resample=Image.NEAREST)
                    depth_gt = depth_gt.crop(crop)
