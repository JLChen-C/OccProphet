    ...
    def __call__(self, results):
        ...
        for key in self.meta_keys:
            if key in results:
                img_metas[key] = results[key]
        # custom modification
        print("results.keys", results.keys(), '\n\r', "num input_dict", len(results['input_dict']), "input_dict[0].keys", results['input_dict'][0].keys())
        # results['input_dict'] has a length of num_frames
        for key in self.meta_keys:
            if key in results['input_dict'][0].keys():
                img_metas[key] = []
                for d in results['input_dict']:
                    img_metas[key].append(d[key])
                assert len(img_metas[key]) == len(results['input_dict'])
